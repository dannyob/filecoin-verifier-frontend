...
export default class WalletProvider extends React.Component<{}, WalletProviderStates> {
    ...
    state = {
        ...
        githubLogged: false,
        githubOcto: {} as any,
        loginGithub: async (code: string, loadClients: boolean) => {
            try {
                ...
                this.state.initGithubOcto(authjson.data.access_token, loadClients)
            } catch (e) {
                this.state.dispatchNotification('Failed to login. Try again later.')
            }
        },
        initGithubOcto: async (token: string, loadClients: boolean) => {
        ...
        loadClientRequests: async () => {
            ...
            const rawIssues = await this.state.githubOcto.issues.listForRepo({
                owner: 'keyko-io',
                repo: 'filecoin-clients-onboarding-test',
        ...
        createRequest: async (data: any) => {
            console.log(this.state.githubOcto)
            try {
                const issue = await this.state.githubOcto.issues.create({
                    owner: 'keyko-io',
                    repo: 'filecoin-clients-onboarding-test ',
    ...
    loadGithub() {
        ...
        if (githubToken && this.state.isLogged) {
            this.state.initGithubOcto(githubToken, true)
        }
    }
